...
// or nil if it was not identified by an X509 certificate. *x509.Certificate
...
type ClientID struct {
	...
	cert  interface{} // 整合 r1 和gm将其改为接口
...
func (c *ClientID) GetID() (string, error) {
	...
	var id string
	if gmCert, ok := c.cert.(*sm2.Certificate); ok {
		id = fmt.Sprintf("x509::%s::%s", getDN(&gmCert.Subject), getDN(&gmCert.Issuer))
	} else if xCert, ok := c.cert.(*x509.Certificate); ok {
		id = fmt.Sprintf("x509::%s::%s", getDN(&xCert.Subject), getDN(&xCert.Issuer))
	} else {
		return "", errors.New("certificate and verify options conversion error.")
	}
...
func (c *ClientID) HasOUValue(OUValue string) (bool, error) {
	...
	if gmCert, ok := c.cert.(*sm2.Certificate); ok {
		for _, OU := range gmCert.Subject.OrganizationalUnit {
			if OU == OUValue {
				return true, nil
			}
		}
	} else if xCert, ok := c.cert.(*x509.Certificate); ok {
		for _, OU := range xCert.Subject.OrganizationalUnit {
			if OU == OUValue {
				return true, nil
			}
		}
	} else {
		return false, errors.New("certificate and verify options conversion error.")
	}
...
// GetX509Certificate returns the X509 certificate associated with the client,
// or nil if it was not identified by an X509 certificate. *x509.Certificate
func (c *ClientID) GetX509Certificate() (interface{}, error) {
	return c.cert, nil
}
...
func (c *ClientID) init() error {
	...
	if gmCert, err := utils.ParseSM2CertificateFromCert(block.Bytes); err == nil {
		c.cert = gmCert
	} else if xCert, err := utils.ParseX509CertificateFromCert(block.Bytes); err == nil {
		c.cert = xCert
	} else {
		return fmt.Errorf("failed to parse certificate: %s", err)
	}
