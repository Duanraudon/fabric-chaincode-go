...
func (hs *clientHandshakeStateGM) handshake() error {
	...
	logger.Debugf("开始向服务端发送信息(版本号:%d,服务名:%s,会话ID:%s,配置版本号：%d)",
		hs.hello.vers,
		hs.hello.serverName,
		hex.EncodeToString(hs.hello.sessionId),
		hs.c.config.MinVersion,
	)
	...
	logger.Debugf("解析来自服务端信息(版本号：%d,服务名：%s,会话ID:%s)", hs.serverHello.vers, hex.EncodeToString(hs.serverHello.sessionId))
...
func (hs *clientHandshakeStateGM) doFullHandshake() error {
	...
	if c.handshakes == 0 {
		...
		for i, asn1Data := range certMsg.certificates {
			...
			if pubKey == nil || pubKey.Curve != sm2.P256Sm2() {
		...
		if !c.config.InsecureSkipVerify {
			...
			for i, cert := range certs {
				// GM SSL 证书链中不含根证书 第1张为签名证书、第2张为加密证书，其他的证书都认为是根证书，只验证签名证书和加密证书
