...
NextCandidate:
	...
	sig, err := priv.Sign(config.rand(), sm3.Sm3Sum(digest), signOpts)
	if err != nil {
...
func (ka *eccKeyAgreementGM) generateServerKeyExchange(config *Config, signCert, cipherCert *Certificate,
	...
	sig, err := priv.Sign(config.rand(), sm3.Sm3Sum(digest), crypto.SHA256)
	if err != nil {
		return nil, err
	}
...
func (ka *eccKeyAgreementGM) processServerKeyExchange(config *Config, clientHello *clientHelloMsg,
	...
	if !sm2PubKey.Verify(sm3.Sm3Sum(digest), sig) {
