...
var (
	logger = flogging.MustGetLogger("gmsm.gmtls")
)
...
func (c *Conn) Read(b []byte) (n int, err error) {
	//logger.Debugf("接收到数据报文:%s", hex.EncodeToString(b))
...
func (c *Conn) Handshake() error {
	...
	if c.isClient {
		logger.Debug("来自客户端握手......")
		c.handshakeErr = c.clientHandshake()
	} else {
		if c.config.GMSupport == nil {
			logger.Debug("来自服务端握手(仅支持TLS方式)......")
			c.handshakeErr = c.serverHandshake()
		} else if c.config.GMSupport.IsAutoSwitchMode() {
			logger.Debug("来自服务端握手来自服务端握手(支持国密SSL和TLS自动切换方式)......")
			c.handshakeErr = c.serverHandshakeAutoSwitch()
		} else {
			logger.Debug("来自服务端握手(仅支持国密SSL方式)......")
