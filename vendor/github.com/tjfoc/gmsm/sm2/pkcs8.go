...
func ParseSm2PublicKey(der []byte) (*PublicKey, error) {
	var pkixPubKey pkixPublicKey

	if _, err := asn1.Unmarshal(der, &pkixPubKey); err != nil {
		return nil, err
	}
	if !reflect.DeepEqual(pkixPubKey.Algo.Algorithm, oidSM2) {
		return nil, errors.New("x509: not sm2 elliptic curve")
	}
	pubKey := PublicKey{}
	pubKey.Curve = P256Sm2()
	pubKey.X, pubKey.Y = elliptic.Unmarshal(pubKey.Curve, pkixPubKey.BitString.Bytes)
	return &pubKey, nil
}
...
func ParseSm2PrivateKey(der []byte) (*PrivateKey, error) {
	...
	if _, err := asn1.Unmarshal(der, &priKey); err != nil {
	...
	newPriKey := new(PrivateKey)
	newPriKey.Curve = curve
	newPriKey.D = k
	...
	newPriKey.X, newPriKey.Y = curve.ScalarBaseMult(privateKey)
	return newPriKey, nil
}
...
func ParsePKCS8UnecryptedPrivateKey(der []byte) (*PrivateKey, error) {
	...
	return ParseSm2PrivateKey(priKey.PrivateKey)
}
...
func MarshalSm2UnecryptedPrivateKey(key *PrivateKey) ([]byte, error) {
	...
	algo.Parameters.FullBytes = []byte{6, 8, 42, 129, 28, 207, 85, 1, 130, 45} // asn1.Marshal(asn1.ObjectIdentifier{1, 2, 156, 10197, 1, 301})
	priKey.Version = 1
	priKey.NamedCurveOID = oidNamedCurveP256SM2
	priKey.PublicKey = asn1.BitString{Bytes: elliptic.Marshal(key.Curve, key.X, key.Y)}
	priKey.PrivateKey = key.D.Bytes()
	r.Version = 0
	r.Algo = algo
	r.PrivateKey, _ = asn1.Marshal(priKey)
	return asn1.Marshal(r)
}
...
func ReadPrivateKeyFromPem(fileName string, pwd []byte) (*PrivateKey, error) {
	...
	return ReadPrivateKeyFromMem(pemBytes, pwd)
}

func ReadPublicKeyFromMem(pemBytes []byte, _ []byte) (*PublicKey, error) {
	if len(pemBytes) == 0 {
	...
	pubKey, err := ParseSm2PublicKey(block.Bytes)
	if err != nil {
		return nil, errors.New("sm2 public key conversion error.")
	}
	if pubKey.Curve != P256Sm2() {
		return nil, errors.New("sm2 public key's curve error.")
	}
	return pubKey, err
}
...
func ReadPublicKeyFromPem(fileName string, pwd []byte) (*PublicKey, error) {
	...
	return ReadPublicKeyFromMem(pemBytes, pwd)
}
